...
import 'package:flame/experimental.dart' as flameExperimental;
...
class SelectLevelGame extends FlameGame with ScrollDetector {
  ...
  FutureOr<void> onLoad() async {
    // lake , peak , column_1 ,column_2 ,big_vulkano ,chain, wall , floor_1
    _atlas = await loadFireAtlas('images/vulkanos.fa');
    ...
    bg.addAll(
      [
        ...[
          AlignComponent(
            alignment: Anchor(0.8, 0.05),
            child: SpriteComponent(
                scale: Vector2(0.5, 0.5), sprite: _atlas.getSprite("column_1")
                // size: Vector2(100, 100),
                ),
          ),
          AlignComponent(
            alignment: Anchor(0.6, 0.9),
            child: SpriteComponent(
                scale: Vector2(0.6, 0.6), sprite: _atlas.getSprite("column_2")
                // size: Vector2(100, 100),
                ),
          ),
          AlignComponent(
          ...
          AlignComponent(
            alignment: Anchor(0.25, 0.34),
            child: SpriteComponent(
                scale: Vector2(0.6, 0.6), sprite: _atlas.getSprite("big_vulkano")
                // size: Vector2(100, 100),
                ),
          ),
        ],
    ...
    camera.setBounds(
      flameExperimental.Rectangle.fromRect(
        Rect.fromLTWH(
          inset.x / 2,
          inset.y / 2,
          (bg.size.x) - inset.x,
...
class SelectLevelButton extends AdvancedButtonComponent {
  ...
  void onMount() {
    super.onMount();
    switch (status) {
      case LevelStatus.disabled:
        isDisabled = true;
      case LevelStatus.enabled:
  ...
  String getNum(String character) {
    if (status == LevelStatus.disabled || status == LevelStatus.enabled) {
  ...
  Future<void> onLoad() async {
    ...
    final labelY =
        status == LevelStatus.enabled || status == LevelStatus.disabled
            ? -32.0
            : 0.0;
    defaultLabel = RectangleComponent(children: [
      ...spriteNum.mapIndexed((index, sprite) => SpriteComponent(
            size: Vector2.all(16),
            position: Vector2(-number.length * 10, labelY) +
                Vector2(20.0 * index, labelY),
            sprite: sprite,
          ))
    ]);
    ...
    if (status != LevelStatus.disabled) {
      ...
      downSkin = SpriteComponent(sprite: skinAsset);
    }
  }
}

class MyParallaxComponent extends ParallaxComponent<SelectLevelGame> {
  @override
  Future<void> onLoad() async {
    parallax = await game.loadParallax(
      [
        ParallaxImageData('stage/01/layers/l1_sky.png'),
      ],
